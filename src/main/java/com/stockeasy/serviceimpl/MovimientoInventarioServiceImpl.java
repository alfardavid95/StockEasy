...
public class MovimientoInventarioServiceImpl implements MovimientoInventarioService {
    ...
    @Override @Transactional(readOnly=true) public List<MovimientoInventario> getMovimientos(String texto, String tipo){ return movRepo.buscar(texto, tipo); }
    @Override @Transactional(readOnly=true) public List<MovimientoInventario> getUltimos(){ return movRepo.findTop5ByOrderByFechaDesc(); }
    @Override @Transactional(readOnly=true) public List<MovimientoInventario> getPorProducto(Integer idProducto){ return movRepo.findByProductoIdProductoOrderByFechaDesc(idProducto); }
    @Override @Transactional public void entrada(Integer idProducto, Integer cantidad, Integer idUsuario, String obs){ registrar(idProducto,cantidad,idUsuario,obs,"Entrada"); }
    @Override @Transactional public void salida(Integer idProducto, Integer cantidad, Integer idUsuario, String obs){ registrar(idProducto,cantidad,idUsuario,obs,"Salida"); }
    private void registrar(Integer idProducto, Integer cantidad, Integer idUsuario, String obs, String tipoNombre){
        ...
        p.setStockActual(nuevo); productoRepo.save(p);
        MovimientoInventario m = new MovimientoInventario();
        m.setProducto(p); m.setUsuario(u); m.setTipoMovimiento(t); m.setCantidad(cantidad); m.setStockAnterior(anterior); m.setStockNuevo(nuevo); m.setObservacion(obs);
        movRepo.save(m);
    }
